...
const ProductCard = ({ product }) => {
  ...
  return (
    <div className="relative group block bg-white border border-ink/10 hover:border-gold/40 hover:-translate-y-1 active:scale-[0.98] hover:shadow-2xl hover:shadow-ink/10 transition-all duration-500 rounded-sm">
      {/* Wishlist Button */}
      <button
        onClick={handleWishlistToggle}
        disabled={loading}
        className="absolute top-4 right-4 z-10 p-2 bg-white/90 backdrop-blur-sm rounded-full shadow-md hover:shadow-lg transition-all duration-300 disabled:opacity-50"
      >
        {isInWishlist ? (
          <HeartSolidIcon className="w-5 h-5 text-oxblood" />
        ) : (
          <HeartIcon className="w-5 h-5 text-ink/40 hover:text-oxblood transition-colors" />
        )}
      </button>
